...
const REDUCE_MOTION_KEY = "pd-reduce-motion";
...
function ModelSection() {
  ...
  return (
    <section className="settings-section surface">
      <h2>模型</h2>
      <p>填入各提供方的 API 密钥即可使用其模型；不配置时为演示模式。</p>
      {active ? (
...
export function SettingsPanel({ onClose, userName, userSource }: { onClose: () => void; userName: string; userSource: "supabase" | "demo" }) {
  ...
  function toggleReduceMotion() {
    ...
    window.localStorage.setItem(REDUCE_MOTION_KEY, next ? "on" : "off");
    setMotionSaved(true);
    window.setTimeout(() => setMotionSaved(false), 2000);
  }

  return (
    <div className="settings-view">
      <button className="text-button settings-back" onClick={onClose} type="button">← 返回</button>

      <section className="settings-section surface">
        <h2>账号</h2>
        <p>账号与训练记录只用于生成你的能力证据。</p>
        <div className="settings-row">
          <div className="settings-row-label"><strong>昵称</strong><span>登录时填写的称呼</span></div>
          <span className="settings-row-value">{userName}</span>
        </div>
        <div className="settings-row">
          <div className="settings-row-label"><strong>身份</strong><span>当前训练角色</span></div>
          <span className="settings-row-value">产品练习生</span>
        </div>
        <div className="settings-row">
          <div className="settings-row-label"><strong>数据存储</strong><span>训练记录保存位置</span></div>
          <span className="settings-row-value">{userSource === "demo" ? "这台电脑（本地运行）" : "Supabase 云端"}</span>
        </div>
      </section>
      ...
      <section className="settings-section surface">
        ...
        <div className="settings-row">
          <div className="settings-row-label">
            <strong>减少界面动画</strong>
            <span>关闭过渡与悬停动画；系统开启「减少动态效果」时默认关闭动画</span>
          </div>
          <span className="settings-row-value">
            {motionSaved ? <span className="settings-saved" role="status">已保存</span> : null}
            <button
